...
#define create_pointer_cast( Type, Name, OriginalSharedPtr )	boost::shared_ptr< Type > Name = boost::dynamic_pointer_cast< Type >( OriginalSharedPtr );
template <class T>
inline boost::shared_ptr<T> create_StoreSQLite( DBConnection& dbConn )
{
	return boost::shared_ptr<T>( new T( dbConn ) );
}
...
class ConnectedVisionModule: 
	public IConnectedVisionModule, 
	public IConnectedVisionWorkerFactory, 
...
public:
	ConnectedVisionModule(const char moduleDescription[], const char inputPinDescription[], const char outputPinDescription[]);
    virtual ~ConnectedVisionModule();
...
public:
	...
	virtual ConnectedVisionModule* getModule() { return this; };
	...
	virtual std::string getConfigHTMLsummary(ConnectedVisionModule &module, const Class_generic_config &config);
...
protected:
	...
	*/
	...
	std::map<id_t, bool> runningSuccessorList; ///< list which config ids of all running successor modules (used to manage control/stop control/reset mechanism when several processing chains that share sub-chains are involved
